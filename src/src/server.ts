...
import { createFeedbackCollector, FeedbackCollector } from './services/feedback-collector';
import { LearningAgent } from './services/agents/learning';
import {
  persistConversation,
...
// Learning infrastructure singletons (initialized in startServer)
let feedbackCollector: FeedbackCollector | null = null;
let learningAgentSingleton: LearningAgent | null = null;
...
// Get quality trend
app.get('/api/quality/trend', requireAuthIfEnabled, async (req, res) => {
  try {
    const { period = 'last_day' } = req.query;
    const { QualityTracker } = await import('./services/quality-tracker');
    const driver = getDriver();
    const tracker = new QualityTracker(driver);
    const trend = await tracker.getQualityTrend(period as string);
    res.json(trend);
  } catch (error) {
    logger.error('API: Failed to fetch quality trend', {
      error: error instanceof Error ? error.message : String(error),
    });
...
// Get quality scorecard for a specific session
app.get('/api/quality/scorecard/:sessionId', requireAuthIfEnabled, async (req, res) => {
  try {
    const { sessionId } = req.params;
    const { QualityTracker } = await import('./services/quality-tracker');
    const driver = getDriver();
    const tracker = new QualityTracker(driver);
    const scorecard = await tracker.getSessionScorecard(sessionId);

    if (!scorecard) {
      return res.status(404).json({ error: 'Quality record not found for this session' });
    }

    res.json(scorecard);
  } catch (error) {
    logger.error('API: Failed to fetch quality scorecard', {
      error: error instanceof Error ? error.message : String(error),
    });
    res.status(500).json({ error: 'Failed to fetch quality scorecard' });
  }
});
...
app.post('/api/feedback', requireAuthIfEnabled, async (req, res) => {
  try {
    ...
    try {
      ...
      // Route through FeedbackCollector for learning (async, non-blocking)
      if (feedbackCollector) {
        const actionMap: Record<string, 'loved' | 'liked' | 'neutral' | 'disliked'> = {
          purchase: 'loved',
          like: 'liked',
          click: 'neutral',
          dislike: 'disliked',
        };
        const action = actionMap[event] || 'neutral';
        feedbackCollector.collectExplicitFeedback({
          recommendation_id: feedbackId,
          user_id: userId,
          recipient_id: 'unknown', // Frontend doesn't send recipientId yet
          product_id: productId,
          action,
          purchased: event === 'purchase',
          would_gift_again: event === 'like' || event === 'purchase',
        }).catch((err: any) => {
          logger.error('FeedbackCollector failed (non-blocking)', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
      }

      // Route through LearningAgent for graph updates (async, non-blocking)
      if (learningAgentSingleton) {
        // Look up the latest Recommendation node for this session to get recommendationId
        const recResult = await session.run(
          `MATCH (r:Recommendation {sessionId: $sessionId})
           RETURN r.id AS recId ORDER BY r.timestamp DESC LIMIT 1`,
          { sessionId }
        );
        const recId = recResult.records.length > 0 ? recResult.records[0].get('recId') : feedbackId;

        const likedIds = event === 'like' ? [productId] : [];
        const dismissedIds = event === 'dislike' ? [productId] : [];
        const purchasedIds = event === 'purchase' ? [productId] : [];
        const clickedIds = event === 'click' ? [productId] : [];

        learningAgentSingleton.process({
          sessionId,
          userId,
          ...
          originalContext: { totalRecommendations: 0, recommendations: [] } as any,
        }).catch((err: any) => {
          logger.error('LearningAgent feedback failed (non-blocking)', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
      }
...
async function startServer() {
  try {
    ...
    const driver = getDriver();
    feedbackCollector = createFeedbackCollector(driver);
    learningAgentSingleton = new LearningAgent(driver);
    logger.info('Server: Learning infrastructure initialized');
