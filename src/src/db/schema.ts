...
export async function createConstraints(): Promise<void> {
  ...
  try {
    ...
    const constraints = [
      ...
      { name: 'quality_record_id', cypher: 'CREATE CONSTRAINT quality_record_id IF NOT EXISTS FOR (qr:QualityRecord) REQUIRE qr.id IS UNIQUE' },
    ];
...
export async function createIndexes(): Promise<void> {
  ...
  try {
    ...
    const indexes = [
      ...
      { name: 'quality_record_timestamp', cypher: 'CREATE INDEX quality_record_timestamp IF NOT EXISTS FOR (qr:QualityRecord) ON (qr.timestamp)' },
      { name: 'quality_record_session', cypher: 'CREATE INDEX quality_record_session IF NOT EXISTS FOR (qr:QualityRecord) ON (qr.sessionId)' },
      // Learning infrastructure indexes
      { name: 'recommendation_session_idx', cypher: 'CREATE INDEX recommendation_session_idx IF NOT EXISTS FOR (rec:Recommendation) ON (rec.sessionId)' },
      { name: 'recommendation_user_idx', cypher: 'CREATE INDEX recommendation_user_idx IF NOT EXISTS FOR (rec:Recommendation) ON (rec.userId)' },
      { name: 'feedback_signal_user_idx', cypher: 'CREATE INDEX feedback_signal_user_idx IF NOT EXISTS FOR (fs:FeedbackSignal) ON (fs.user_id)' },
      { name: 'feedback_timestamp_idx', cypher: 'CREATE INDEX feedback_timestamp_idx IF NOT EXISTS FOR (f:Feedback) ON (f.timestamp)' },
    ];
